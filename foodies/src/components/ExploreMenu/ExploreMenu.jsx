  ...
  return (
    <div className="explore-menu position-relative">
      <h1 className="d-flex align-items-center justify-content-between">Explore our menu
        ...
        <i className='bi bi-arrow-left-circle scroll-icon' onClick={scrollLeft} style={{ color: 'black' }}></i>
        <i className='bi bi-arrow-right-circle scroll-icon' onClick={scrollRight} style={{ color: 'black' }}></i>
      </div>
      </h1>
      <p style={{ fontSize: "1.2rem" }}>Explore curated lists of dishes from top categories</p>
          ...
          categories.map((item,index)=>{
            return(
              <div key={index} className='text-center explore-menu-list-item' onClick={()=>setCategory(prev=>prev===item.category?'All':item.category)}>
                ...
                <p className='mt-2 ' style={{ color: "black" }}>{item.category}</p>
              </div>
            )
          })
        }
